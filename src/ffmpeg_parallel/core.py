...
def generate_temp_directory(video_file: str) -> tuple[str, str, str]:
    ...
    file_hash = hashlib.md5(str.encode(
        os.path.split(video_file)[-1])).hexdigest()
...
def get_video_info(video_file: str) -> dict:
    ...
    result = subprocess.run(ffprobe_info_command,
...
def encode_chunk(args) -> None:
    ...
    output_file = os.path.join(
        encoded_chunks_dir, os.path.basename(chunk_file))
    progress_log = os.path.join(
        encoded_chunks_dir, f"{chunk_name}_progress.log")
    ...
    if codec in ['libx264', 'libx265']:
        ffmpeg_encode_command.extend(
            ["-preset", encoding_options['preset'], "-crf", str(encoding_options['crf'])])
    elif codec == 'libsvtav1':
        ffmpeg_encode_command.extend(
            ["-preset", encoding_options['preset'], "-qp", str(encoding_options['qp'])])
    if encoding_options["merge_audio"]:
        ...
        ffmpeg_encode_command.extend(
            ["-filter_complex", f"amerge=inputs={audio_tracks}", "-c:a", "flac", "-b:a", "320k"])
    else:
    ...
    process = subprocess.Popen(
        ffmpeg_encode_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def merge_chunks(chunks: list[str], temp_dir: str, output_file: str) -> None:
    ...
    subprocess.run(ffmpeg_merge_command, stdout=subprocess.DEVNULL,
                   stderr=subprocess.DEVNULL, check=True)
...
def print_size_comparison(original_file: str, comparison_file: str) -> None:
    ...
    try:
        ...
        if original_size > 0:
            ...
            print(
                f"Encoded file size: {comparison_size / (1024*1024):.2f} MiB")
...
def run_ffmpeg_parallel(video_file: str, output_file: str, codec: str, workers: int, threads_per_worker: int, encoding_options: dict):
    ...
    if not output_file:
        ...
        # Fallback to raw codec name if not found
        codec_suffix: str = codec_display_names.get(codec, codec)

        input_dir = os.path.dirname(os.path.abspath(video_file))
        base_name = os.path.splitext(os.path.basename(video_file))[0]
        video_ext = '.mp4'  # os.path.splitext(video_file)[1]
        output_file = os.path.join(
            input_dir, f"{base_name} [{codec_suffix}]{video_ext}")

    temp_dir, chunks_dir, encoded_chunks_dir = generate_temp_directory(
        output_file)
    ...
    numerator: int
    denominator: int
    numerator, denominator = (int(x) for x in video_info.get(
    ...
    for chunk in chunk_files:
        ...
        estimated_total_frames.append(
            math.ceil(chunk_duration * avg_frame_rate))
    ...
    encode_args = [
        (chunk, encoded_chunks_dir, codec, threads_per_worker, position,
         encoding_options, estimated_total_frames[position], progress_queue)
